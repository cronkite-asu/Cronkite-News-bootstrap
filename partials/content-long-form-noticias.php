...
function generateByline($currPostID, $currIntro, $publishDate, $style)
{
    ...
    if (have_rows('byline_info')) {
        ...
        echo '<span class="author_name">Por ';
    ...
    ?>
        ...
        <div class="large-4 medium-4 small-12 cell story-credits-date medium-text-right text-left">
          <ul class="top_social">
            <li><a href="https://twitter.com/intent/tweet?url=<?php echo get_permalink($currPostID); ?>&text=<?php echo urlencode(strip_tags($currIntro)); ?>" target="_blank"><i class="fa-brands fa-x-twitter"></i></a></li>
...
<div id="intro-head-photo" class="grid-container full">
  ...
  <div class="grid-container video-content <?php echo $videoWidth; ?>">
    ...
    <div class="grid-container photo-content single <?php echo $photoWidth; ?>">
      <div class="grid-x grid-padding-x">
            ...
            if (have_rows('photos')) {
                while (have_rows('photos')) {
                    ...
                    if (get_sub_field('caption') != '') {
                        if ($captionCounter == 0 && get_sub_field('caption') != '') {
                            $combinedCaption = '<strong>Izquierda:</strong> '. strip_tags(get_sub_field('caption'), '<a>');
                        } elseif ($captionCounter == 1 && get_sub_field('caption') != '') {
                            $combinedCaption .= ' <strong>Centrar:</strong> '. strip_tags(get_sub_field('caption'), '<a>');
                        } elseif ($captionCounter == 2 && get_sub_field('caption') != '') {
                            $combinedCaption .= ' <strong>Derecha:</strong> '. strip_tags(get_sub_field('caption'), '<a>');
                        }
                        $captionCounter++;
                    }
                    ?>
                ...
                if ($captionCounter == 1) {
                    $combinedCaption = str_replace('<strong>Izquierda:</strong>', '', $combinedCaption);
                }
                ?>
        <div class="large-12 cell">
    ...
    <div class="grid-container text-content charts-doc <?php echo $fullWidth; ?>">
      <div class="grid-x grid-padding-x">
        <div class="large-12 cell">
            ...
            if (have_rows('photos')) {
                while (have_rows('photos')) {
                    ...
                    if ($photoCounter >= 4) {
                        if (get_sub_field('caption') != '') {
                            if ($captionCounter == 0 && get_sub_field('caption') != '') {
                                $combinedCaption = '<strong>From Izquierda:</strong> '. strip_tags(get_sub_field('caption'), '<a>');
                            } elseif ($captionCounter == 1 && get_sub_field('caption') != '') {
                                $combinedCaption .= strip_tags(get_sub_field('caption'), '<a>');
                            } elseif ($captionCounter == 2 && get_sub_field('caption') != '') {
                                $combinedCaption .= strip_tags(get_sub_field('caption'), '<a>');
